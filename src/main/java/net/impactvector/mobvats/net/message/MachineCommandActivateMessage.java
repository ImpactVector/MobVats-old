package net.impactvector.mobvats.net.message;
 ...
 * @see net.impactvector.mobvats.common.multiblock.interfaces.IActivateable
	...
	public static class Handler extends ModTileEntityMessageHandlerServer<MachineCommandActivateMessage> {
		...
		protected void processTileEntityMessage(MachineCommandActivateMessage message, MessageContext ctx, TileEntity tileEntity) {
			...
			if (tileEntity instanceof IActivateable) {
				...
				ModEventLog.error("Received a MachineCommandActivateMessage for %d, %d, %d but found no activateable machine", position.getX(), position.getY(), position.getZ());
			}
		}
	}
}
