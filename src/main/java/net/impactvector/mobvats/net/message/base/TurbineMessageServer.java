		...
		protected void processTileEntityMessage(MessageT message, MessageContext ctx, TileEntity tileEntity) {
			...
			if (tileEntity instanceof TileEntityTurbinePartBase) {
				...
				if (null != turbine) {
					...
					ModEventLog.error("Received TurbineMessageServer for a turbine part @ %d, %d, %d which has no attached turbine",
							position.getX(), position.getY(), position.getZ());
				}
			} else if (null != position) {

				ModEventLog.error("Received TurbineMessageServer for a non-turbine-part block @ %d, %d, %d",
						position.getX(), position.getY(), position.getZ());
			}
		}
