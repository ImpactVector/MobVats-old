...
import net.impactvector.mobvats.common.multiblock.tileentity.TileEntityReactorPartBase;
		...
		protected void processTileEntityMessage(MessageT message, MessageContext ctx, TileEntity tileEntity) {
			...
			if (tileEntity instanceof TileEntityReactorPartBase) {
				...
				if (null != reactor) {
					...
					ModEventLog.error("Received ReactorMessageServer for a reactor part @ %d, %d, %d which has no attached reactor",
							position.getX(), position.getY(), position.getZ());
				}
			} else if (null != position) {

				ModEventLog.error("Received ReactorMessageServer for a non-reactor-part block @ %d, %d, %d",
						position.getX(), position.getY(), position.getZ());
			}
		}
