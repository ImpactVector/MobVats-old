...
public class BlockBROre extends BlockBR {
	...
	public void registerRecipes() {
		...
		if (MobVats.CONFIG.registerYelloriteSmeltToUranium) {

			product = OreDictionaryHelper.getOre("ingotUranium");

			if (null == product) {

				ModEventLog.warning("Config value registerYelloriteSmeltToUranium is set to True, but there are no ores registered as ingotUranium in the ore dictionary! Falling back to using standard yellorium only.");
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		return null == mineral ? super.getItemDropped(state, rand, fortune) : ModItems.minerals;
	}
