...
import net.impactvector.mobvats.common.interfaces.IReactorFuelInfo;
import net.impactvector.mobvats.common.multiblock.block.BlockMultiblockFlesh;
import net.impactvector.mobvats.common.multiblock.block.BlockMultiblockHead;
import net.impactvector.mobvats.common.multiblock.helpers.*;
...
import net.impactvector.mobvats.net.message.multiblock.ReactorUpdateMessage;
import net.impactvector.mobvats.net.message.multiblock.ReactorUpdateWasteEjectionMessage;
...
//    protected Iterator<TileEntityReactorFuelRod> currentFuelRod;
    protected Iterator<TileEntityReactorFlesh> currentFlesh;
	int reactorVolume;
	...
	public Set<TileEntityReactorHead> attachedHeads;
	public Set<TileEntityReactorFlesh> attachedFlesh;
...
//	private FuelAssembly[] _fuelAssemblies;
	...
	public MultiblockReactor(World world) {
		...
		attachedHeads = new HashSet<TileEntityReactorHead>();
		attachedFlesh = new HashSet<TileEntityReactorFlesh>();
		attachedGlass = new HashSet<TileEntityReactorGlass>();
//		this._fuelAssemblies = null;
        upgradeSetup = new UpgradeSetup();
...
//        learnTicksOffset = Settings.learnTicks + MobVats.RANDOM.nextInt(11);
        learnTicksOffset = 20 + MobVats.RANDOM.nextInt(11);
...
//		}
        ...
        if(part instanceof TileEntityReactorHead) {
            attachedHeads.add((TileEntityReactorHead)part);
        }

        if(part instanceof TileEntityReactorFlesh) {
            attachedFlesh.add((TileEntityReactorFlesh)part);
            currentFlesh = attachedFlesh.iterator();
        }
...
//		}
        ...
        if(part instanceof TileEntityReactorHead) {
            attachedHeads.remove(part);
        }

        if(part instanceof TileEntityReactorFlesh) {
            attachedFlesh.remove(part);
        }
...
//
//					if (ModBlocks.reactorCasing != state.getBlock()) {
//
...
//		}
    ...
    void onSpawn(MobAssembly mob) {
        produceOutput(mob);
//        if (consumedRf >= mob.spawnReq.getTotalRf()) {
//
//            produceOutput();
//
//            /**
...
//            setConsumedRf(0);
//        } else {
//            if (MobVats.CONFIG.strictPower)
//                setConsumedRf(0);
//        }
    }

    private void produceOutput(MobAssembly mob) {

	    World worldObj = mob.getMobHead().getWorld();
        SpawnerManager.SpawnLoot loot = MobVats.spawnerManager.getSpawnerLoot(mob.getMobHead().getMobName(), upgradeSetup
                , worldObj.getDifficultyForLocation(mob.getMobHead().getPos()));

        List<IItemHandler> validHandlers = new ArrayList<>();

        for (TileEntityReactorAccessPort p : attachedAccessPorts) {
            if (p.getDirection() == IInputOutputPort.Direction.Output)
            {
                validHandlers.add(p.getItemStackHandler(false));
            }
        }
...
//            }
	...
	private void tryPickupModItems(MobAssembly mob) {

		EnumEnchantKey key = upgradeSetup.getEnchantKey();
		TileEntityReactorHead head = mob.getMobHead();
		String mobName = head.getMobName();

        /* If still learning check for any dropped items */
		if (!MobVats.LOOT_TABLE_MANAGER.isFull(mobName, key)) {

			if (mob.lootLocation == null) {
				BlockPos checkPos = new BlockPos(head.getPos().getX(), 1, head.getPos().getZ());
				int range = 2;
                mob.lootLocation = new AxisAlignedBB(checkPos).expand(range, 0, range);
			}

			List<EntityItem> itemList = head.getWorld().getEntitiesWithinAABB(EntityItem.class, mob.lootLocation, EntitySelectors.IS_ALIVE);
			if (!itemList.isEmpty()) {
				MobVats.LOOT_TABLE_MANAGER.update(mobName, key, itemList, false);
	...
	public boolean updateServer() {
        ...
        //DO THE THING
        for (MobAssembly mob: this._mobAssemblies) {

			tryPickupModItems(mob);

            mob.currLearnTicks++;
            if (mob.currLearnTicks >= learnTicksOffset) {
                if (!MobVats.LOOT_TABLE_MANAGER.isFull(mob.getMobHead().getMobName(), upgradeSetup.getEnchantKey())) {
                /* Not full so fake another spawn */
                    BlockPos spawnPos = new BlockPos(mob.getMobHead().getPos().getX(), 0, mob.getMobHead().getPos().getZ());
                    MobVats.spawnerManager.spawn(mob.getMobHead().getMobName(), upgradeSetup.getEnchantKey(), mob.getMobHead().getWorld(), spawnPos);
                }
                mob.currLearnTicks = 0;
            }

            /* Do we have any info on this mob yet - should only happen until the first event fires */
            if (MobVats.LOOT_TABLE_MANAGER.isEmpty(mob.getMobHead().getMobName(), upgradeSetup.getEnchantKey()))
                return true;
            ...
            if(getActive()) {
                ...
                if (mob.currSpawnTicks >= mob.spawnReq.getSpawnTime()) {
                    onSpawn(mob);
                    mob.currSpawnTicks = 0;
                }
            }
        }
...
//            this.updateFuelAssembliesQuota();
	...
	protected void generateEnergy(float newEnergy) {
		newEnergy = newEnergy * MobVats.CONFIG.powerProductionMultiplier * MobVats.CONFIG.reactorPowerProductionMultiplier;
...
//				block == Blocks.DIAMOND_BLOCK || block == Blocks.EMERALD_BLOCK
        ...
        if (block == ModBlocks.vatHead) {
            boolean found = false;
            for (TileEntityReactorHead head : this.attachedHeads) {
            ...
            if (!found)
                this.attachedHeads.add((TileEntityReactorHead) ModBlocks.vatHead.createTileEntity(world, blockState));

            return true;
        }

        if (block == ModBlocks.vatFlesh) {
            boolean found = false;
            for (TileEntityReactorFlesh flesh : this.attachedFlesh) {
            ...
            if (!found) {
                this.attachedFlesh.add((TileEntityReactorFlesh) ModBlocks.vatFlesh.createTileEntity(world, blockState));
                currentFlesh = attachedFlesh.iterator();
            }
    ...
    protected boolean doesPartBelong(IMultiblockPart part) {
        if (part instanceof TileEntityReactorHead
                || part instanceof TileEntityReactorFlesh
                || part instanceof BlockMultiblockFlesh
                || part instanceof BlockMultiblockHead)
            return true;
        else
...
//		this._fuelAssemblies = null;
	...
	protected void onAssimilate(MultiblockControllerBase otherMachine) {
		if(!(otherMachine instanceof MultiblockReactor)) {
			ModEventLog.warning("[%s] Reactor @ %s is attempting to assimilate a non-Reactor machine! That machine's data will be lost!", WORLD.isRemote?"CLIENT":"SERVER", getReferenceCoord());
			return;
		}
...
//		}
	...
	protected int getMaximumXSize() {
		return MobVats.CONFIG.maxReactorSize;
	}
	...
	protected int getMaximumYSize() {
		return MobVats.CONFIG.maxReactorHeight;
	}
...
//	}
    public int getMobAssembliesCount() {
        return null != this._mobAssemblies ? this._mobAssemblies.length : 0;
    }
...
//	}
    ...
    private void rebuildMobAssemblies() {

        int count = this.attachedHeads.size();
        int idx = 0;

        this._mobAssemblies = new MobAssembly[count];

        for (TileEntityReactorHead head : this.attachedHeads)
            this._mobAssemblies[idx++] = new MobAssembly(head);
    }
