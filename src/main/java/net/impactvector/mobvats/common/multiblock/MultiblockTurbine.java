...
import net.impactvector.mobvats.common.multiblock.block.ITurbineRotorPart;
import net.impactvector.mobvats.common.multiblock.helpers.FloatUpdateTracker;
...
import net.impactvector.mobvats.gui.container.ISlotlessUpdater;
...
import net.impactvector.mobvats.net.message.multiblock.TurbineUpdateMessage;
...
//
//				if (null == resource || ModFluids.fluidSteam != resource.getFluid())
//					return 0;
//
...
//		};
	...
	public void onConfigChanged() {
		...
		MultiblockTurbine.inductorBaseDragCoefficient = 0.1f * MobVats.CONFIG.turbineCoilDragMultiplier;
		MultiblockTurbine.inputFluidPerBlade = (int) Math.floor(BASE_FLUID_PER_BLADE * MobVats.CONFIG.turbineFluidPerBladeMultiplier);
	...
	protected boolean isMachineWhole(IMultiblockValidator validatorCallback) {
		...
		final boolean rotorEndValid = (te instanceof TileEntityTurbinePart); //&& ModBlocks.turbineHousing == te.getBlockType();
	...
	protected int getMaximumXSize() {
		return MobVats.CONFIG.maxTurbineSize;
	}
	...
	protected int getMaximumYSize() {
		return MobVats.CONFIG.maxTurbineHeight;
	}
	...
	protected void onAssimilate(MultiblockControllerBase otherMachine) {
		if(!(otherMachine instanceof MultiblockTurbine)) {
			ModEventLog.warning("[%s] Turbine @ %s is attempting to assimilate a non-Turbine machine! That machine's data will be lost!", WORLD.isRemote?"CLIENT":"SERVER", getReferenceCoord());
			return;
		}
	...
	*/
	...
	public void deserialize(ByteBuf buf) {
		...
		if(inputFluidID.isEmpty() || inputFluidAmt <= 0) {
			...
			if(fluid == null) {
				ModEventLog.warning("[CLIENT] Multiblock Turbine received an unknown fluid of type %d, setting input tank to empty", inputFluidID);
				this._inputTank.setFluid(null);
			}
			else {
		...
		if(outputFluidID.isEmpty() || outputFluidAmt <= 0) {
			...
			if(fluid == null) {
				ModEventLog.warning("[CLIENT] Multiblock Turbine received an unknown fluid of type %d, setting output tank to empty", outputFluidID);
				this._outputTank.setFluid(null);
			}
			else {
	...
	*/
	...
	protected void generateEnergy(float newEnergy) {
		newEnergy = newEnergy * MobVats.CONFIG.powerProductionMultiplier * MobVats.CONFIG.turbinePowerProductionMultiplier;
	...
	private void recalculateDerivedStatistics() {
		...
		frictionalDrag = rotorMass * rotorDragCoefficient * MobVats.CONFIG.turbineMassDragMultiplier;
		bladeDrag = baseBladeDragCoefficient * bladeSurfaceArea * MobVats.CONFIG.turbineAeroDragMultiplier;
