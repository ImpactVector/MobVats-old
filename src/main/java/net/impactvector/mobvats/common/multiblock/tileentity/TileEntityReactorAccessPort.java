...
import net.impactvector.mobvats.client.gui.GuiReactorAccessPort;
import net.impactvector.mobvats.common.ModEventLog;
import net.impactvector.mobvats.common.ItemHandlerWrapper;
...
import net.impactvector.mobvats.gui.container.ContainerReactorAccessPort;
...
public class TileEntityReactorAccessPort extends TileEntityReactorPart implements INeighborUpdatableEntity, IInputOutputPort {
	...
	public int emitReactant(String reactantType, int amount) {
		...
		if(bestMapping == null) {
			ModEventLog.warning("There are no mapped item types for reactant %s. Using cyanite instead.", reactantType);
		...
		if(newItem == null) {
			ModEventLog.warning("Could not find item for oredict entry %s, using cyanite instead.", bestMapping.getSource());
			newItem = ModItems.ingotMetals.createItemStack(MetalType.Cyanite, 1);
		}
		else {
