...
import net.impactvector.mobvats.common.multiblock.interfaces.IConditionalUpdater;
		...
		for(int i = 0; i < tankNames.length; i++) {
			...
			if(hasReactant) {
				...
				if(!Reactants.isKnown(reactantName)) {
					ModEventLog.warning("Read an unknown reactant <%s> from a network message; tank %s will remain empty", reactantName, tankNames[i]);
				}
				else {
